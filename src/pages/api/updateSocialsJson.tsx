...
const updateCreatorSocials = async (socials: Social[]) => {
  const new_creators = creators.map((creator: Creator, index: number) => {
    const socials_filter = socials.filter(
      //@ts-ignore
      (social) => social.creator_username === collections[index].creator_id
    );
    ...
    // creator.discord_members = discord_members;
    return creator;
  });
  return new_creators as Creator[];
};
const updateCollectionSocials = async (socials: Social[]) => {
  const new_collections = collections.map((collection: any, index: number) => {
    const socials_filter = socials.filter(
      //@ts-ignore
      (social) => social.creator_username === collection.creator_id
    );
    const twitter_followers = socials_filter[0]
      ? socials_filter[0].twitter_followers
      : null;
...
export const getCreatorSocials = async () => {
  ...
  for (let index = 0; index < creators.length; index++) {
    ...
    twitter_data =
      twitter_data != undefined && twitter_data != false && twitter_data.data;
