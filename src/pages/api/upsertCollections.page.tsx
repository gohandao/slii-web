...
const upsertData = async (collections: Collection[]) => {
  ...
  collections.map(async (collection, index) => {
    setTimeout(async () => {
      ...
      //1.insert social
      const OSUser = await getOSUser(collection.creator_address);
      ...
      await fetch(`https://api.opensea.io/api/v1/collection/${collection.slug}`, options)
        .then((response) => {
        ...
        .then(async (response) => {
          ...
          data.creator_address = collection.creator_address;
